...
export const authorizeRoles = () => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const userRole = roles.find(role => role.id === req.user?.role_id) as Role;
      if (!userRole) {
        throw new Error("Role not found");
      }

      const userPermission = await getPermissionById(userRole.permission_id) as Permission;
      if (!userPermission) {
      ...
      res.status(403).json({
        message: (error as Error).message || "Access denied. Role or permission not found.",
      });
    }
  };
}; 