...
import { calculateScore } from "../utils/scoreLogic.js";

export const submitKahootAttemptService = (model: {
    ...
    const formDetail = await formKahootModel.getAllDataFormKahoots(payload.form_uuid);
    ...
    const answersWithId = payload.attempt_answers.map((answer) => {
      ...
      if (!questionId) {
        throw new AppError(`Internal Error: UUID for a question was not found.`, 404);
      }
      return {
    ...
    const totalScore = answersWithId.reduce((sum, answer) => {
...
export const getQuizHistoryService = (
  ...
  return async (user_uuid: string) => {
    const userDetail = await userModel.getDetails(user_uuid);
    const quiz = await quizKahootModel.getHistory(userDetail.id);
    return quiz;
  };
};