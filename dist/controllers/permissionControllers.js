...
import { createPermissionService, getAllPermissionsService, getPermissionByIdService, updatePermissionService, deletePermissionService, } from "../services/permissionServices.js";
import { pickKey } from "../utils/queryHelper.js";
import { ValidateSchema } from "../utils/validateSchema.js";
import { permissionBodySchema, permissionParamsSchema, } from "../schemas/permissionSchema.js";
...
export const UpdatePermissionController = async (req, res) => {
    try {
        ...
        const { route } = ValidateSchema(permissionBodySchema, req.body);
        const permissionModel = new PermissionModel(pool);
        const permission = await updatePermissionService(permissionModel)(id, Array.isArray(route) ? route[0] : route);
        if (!permission) {
            res.send({
                code: 404,
                message: "Permission not found",
